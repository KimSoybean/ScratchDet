...
def ConvBNLayer(net, from_layer, out_layer, use_bn, use_relu, num_output,
    ...
    scale_prefix='', scale_postfix='_scale', bias_prefix='', bias_postfix='_bias',bbbb=False,
    **bn_params):
  if use_bn:
...
def VGGNetBNBody(net, from_layer, need_fc=True, fully_conv=False, reduced=False,
        dilated=False, nopool=False, dropout=True, freeze_layers=[], dilate_pool4=False):
		
    def bn_relu(bottom, inplace=False):
        batch_norm = L.BatchNorm(bottom, in_place=inplace,
    ...
    assert from_layer in net.keys()
    net.conv1_1 = L.Convolution(net[from_layer], num_output=64, pad=1, kernel_size=3, **kwargs)
    temp = bn_relu(net.conv1_1,inplace=True)
    net.conv1_2 = L.Convolution(temp, num_output=64, pad=1, kernel_size=3, **kwargs)
    temp = bn_relu(net.conv1_2,inplace=True)


    net.pool1 = L.Pooling(temp, pool=P.Pooling.MAX, kernel_size=2, stride=2)
    temp = bn_relu(net.pool1,inplace=True)

    net.conv2_1 = L.Convolution(temp, num_output=128, pad=1, kernel_size=3, **kwargs)
    temp = bn_relu(net.conv2_1,inplace=True)
    net.conv2_2 = L.Convolution(temp, num_output=128, pad=1, kernel_size=3, **kwargs)
    temp = bn_relu(net.conv2_2,inplace=True)


    net.pool2 = L.Pooling(temp, pool=P.Pooling.MAX, kernel_size=2, stride=2)
    temp = bn_relu(net.pool2,inplace=True)
	
    net.conv3_1 = L.Convolution(temp, num_output=256, pad=1, kernel_size=3, **kwargs)
    temp = bn_relu(net.conv3_1,inplace=True)
    net.conv3_2 = L.Convolution(temp, num_output=256, pad=1, kernel_size=3, **kwargs)
    temp = bn_relu(net.conv3_2,inplace=True)
    net.conv3_3 = L.Convolution(temp, num_output=256, pad=1, kernel_size=3, **kwargs)
    temp = bn_relu(net.conv3_3,inplace=True)
	
    net.pool3 = L.Pooling(temp, pool=P.Pooling.MAX, kernel_size=2, stride=2)
    temp = bn_relu(net.pool3,inplace=True)
	
    net.conv4_1 = L.Convolution(temp, num_output=512, pad=1, kernel_size=3, **kwargs)
    temp = bn_relu(net.conv4_1,inplace=True)
    net.conv4_2 = L.Convolution(temp, num_output=512, pad=1, kernel_size=3, **kwargs)
    temp = bn_relu(net.conv4_2,inplace=True)
    net.conv4_3 = L.Convolution(temp, num_output=512, pad=1, kernel_size=3, **kwargs)
    temp = bn_relu(net.conv4_3,inplace=True)

    net.pool4 = L.Pooling(temp, pool=P.Pooling.MAX, kernel_size=2, stride=2)
    dilation = 1
    kernel_size = 3
    pad = int((kernel_size + (dilation - 1) * (kernel_size - 1)) - 1) / 2
    temp = bn_relu(net.pool4,inplace=True)
    
    net.conv5_1 = L.Convolution(temp, num_output=512, pad=pad, kernel_size=kernel_size, dilation=dilation, **kwargs)
    temp = bn_relu(net.conv5_1,inplace=True)
    
    net.conv5_2 = L.Convolution(temp, num_output=512, pad=pad, kernel_size=kernel_size, dilation=dilation, **kwargs)
    temp = bn_relu(net.conv5_2,inplace=True)
    
    net.conv5_3 = L.Convolution(temp, num_output=512, pad=pad, kernel_size=kernel_size, dilation=dilation, **kwargs)
    temp = bn_relu(net.conv5_3,inplace=True)
	
    net.pool5 = L.Pooling(temp, pool=P.Pooling.MAX, pad=1, kernel_size=3, stride=1)
    temp = bn_relu(net.pool5,inplace=True)

    dilation = dilation * 6
    kernel_size = 3
    num_output = 1024
    pad = int((kernel_size + (dilation - 1) * (kernel_size - 1)) - 1) / 2
    net.fc6 = L.Convolution(temp, num_output=num_output, pad=pad, kernel_size=kernel_size, dilation=dilation, **kwargs)
    temp = bn_relu(net.fc6,inplace=True)

    net.fc7 = L.Convolution(temp, num_output=1024, kernel_size=1, **kwargs)
    temp = bn_relu(net.fc7,inplace=True)

    net.conv6_1 = L.Convolution(temp, num_output=256, pad=0, kernel_size=1, **kwargs)
    temp = bn_relu(net.conv6_1,inplace=True)
    net.conv6_2 = L.Convolution(temp, num_output=512, pad=1, kernel_size=3, stride=2, **kwargs)
    temp = bn_relu(net.conv6_2,inplace=True)
	
    net.conv7_1 = L.Convolution(temp, num_output=128, pad=0, kernel_size=1, **kwargs)
    temp = bn_relu(net.conv7_1,inplace=True)
    net.conv7_2 = L.Convolution(temp, num_output=256, pad=1, kernel_size=3, stride=2, **kwargs)
    temp = bn_relu(net.conv7_2,inplace=True)

    net.conv8_1 = L.Convolution(temp, num_output=128, pad=0, kernel_size=1, **kwargs)
    temp = bn_relu(net.conv8_1,inplace=True)
    net.conv8_2 = L.Convolution(temp, num_output=256, pad=0, kernel_size=3, stride=1, **kwargs)
    temp = bn_relu(net.conv8_2,inplace=True)

    net.conv9_1 = L.Convolution(temp, num_output=128, pad=0, kernel_size=1, **kwargs)
    temp = bn_relu(net.conv9_1,inplace=True)
    net.conv9_2 = L.Convolution(temp, num_output=256, pad=0, kernel_size=3, stride=1, **kwargs)
    temp = bn_relu(net.conv9_2,inplace=True)
...
def Resnet34rootV2_res(net, from_layer,freeze_layers=[]):
    def bn_relu(bottom, inplace=False):
        batch_norm = L.BatchNorm(bottom, in_place=inplace,
                                 param=[dict(lr_mult=0, decay_mult=0), dict(lr_mult=0, decay_mult=0),
                                        dict(lr_mult=0, decay_mult=0)])
        ...
        relu = L.ReLU(scale, in_place=True)
        return relu
    def res_relu(branch1,branch2):
        Elt= L.Eltwise(branch1, branch2)
        relu = L.ReLU(Elt, in_place=True)
        return relu
    def bn_scale_res_relu(bottom,branch):
        batch_norm = L.BatchNorm(bottom, in_place=True,
                                 param=[dict(lr_mult=0, decay_mult=0), dict(lr_mult=0, decay_mult=0),
                                        dict(lr_mult=0, decay_mult=0)])
        scale = L.Scale(batch_norm, bias_term=True, in_place=True, filler=dict(value=1), bias_filler=dict(value=0))
        Elt= L.Eltwise(branch, scale)
        relu = L.ReLU(Elt, in_place=True)
        return relu
    def bn_scale(bottom, inplace=True):
        batch_norm = L.BatchNorm(bottom, in_place=inplace,
                                 param=[dict(lr_mult=0, decay_mult=0), dict(lr_mult=0, decay_mult=0),
                                        dict(lr_mult=0, decay_mult=0)])
        scale = L.Scale(batch_norm, bias_term=True, in_place=True, filler=dict(value=1), bias_filler=dict(value=0))
	...
	#stem block
    assert from_layer in net.keys()
    net.stem_1 = L.Convolution(net[from_layer], num_output=64, pad=1,  kernel_size=3,stride=1, **kwargs)
    temp = bn_relu(net.stem_1,inplace=True)
    net.stem_2 = L.Convolution(temp, num_output=64, pad=1, kernel_size=3, **kwargs)
    temp = bn_relu(net.stem_2,inplace=True)
    net.stem_3 = L.Convolution(temp, num_output=64, pad=1, kernel_size=3, **kwargs)
    temp = bn_relu(net.stem_3,inplace=True)
    net.pool1 = L.Pooling(temp, pool=P.Pooling.MAX, kernel_size=2, stride=2)
	#block1
    net.conv2_1 = L.Convolution(net.pool1, num_output=64, pad=1,kernel_size=3, **kwargs)
    temp=bn_relu(net.conv2_1,inplace=True)
    net.conv2_2 = L.Convolution(temp, num_output=64, pad=1,kernel_size=3, **kwargs)
    temp1=L.Eltwise(net.pool1,net.conv2_2)

    temp = bn_relu(temp1)
    net.conv2_3 = L.Convolution(temp, num_output=64, pad=1,kernel_size=3, **kwargs)
    temp=bn_relu(net.conv2_3,inplace=True)
    net.conv2_4 = L.Convolution(temp, num_output=64, pad=1,kernel_size=3, **kwargs)    
    temp2=L.Eltwise(temp1,net.conv2_4)
	
    temp = bn_relu(temp2)
    net.conv2_5 = L.Convolution(temp, num_output=64, pad=1,kernel_size=3, **kwargs)
    temp=bn_relu(net.conv2_5,inplace=True)
    net.conv2_6 = L.Convolution(temp, num_output=64, pad=1,kernel_size=3, **kwargs)    
    temp111=L.Eltwise(temp2,net.conv2_6)
    
	#block2
    temp = bn_relu(temp111)
    match1=L.Convolution(temp, num_output=128, pad=0 , kernel_size=1, stride=2,**kwargs)
    net.conv3_1 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3,stride=2 ,**kwargs)
    temp=bn_relu(net.conv3_1,inplace=True)
    net.conv3_2 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3, **kwargs)
    temp3=L.Eltwise(match1,net.conv3_2)

    temp = bn_relu(temp3)
    net.conv3_3 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3, **kwargs)
    temp=bn_relu(net.conv3_3,inplace=True)
    net.conv3_4 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3, **kwargs)    
    temp4=L.Eltwise(temp3,net.conv3_4)
	
    temp = bn_relu(temp4)
    net.conv3_5 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3, **kwargs)
    temp=bn_relu(net.conv3_5,inplace=True)
    net.conv3_6 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3, **kwargs)    
    temp221=L.Eltwise(temp4,net.conv3_6)

    temp = bn_relu(temp221)
    net.conv3_7 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3, **kwargs)
    temp=bn_relu(net.conv3_7,inplace=True)
    net.conv3_8 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3, **kwargs)    
    temp222=L.Eltwise(temp221,net.conv3_8)	

	#block3
    temp = bn_relu(temp222)
    match2=L.Convolution(temp, num_output=256, pad=0,kernel_size=1, stride=2, **kwargs)
    net.conv4_1 = L.Convolution(temp, num_output=256, pad=1,kernel_size=3, stride=2, **kwargs)
    temp=bn_relu(net.conv4_1,inplace=True)
    net.conv4_2 = L.Convolution(temp, num_output=256, pad=1,kernel_size=3, **kwargs)
    temp5=L.Eltwise(match2,net.conv4_2)
    	
    temp = bn_relu(temp5)
    net.conv4_3 = L.Convolution(temp, num_output=256, pad=1,kernel_size=3, **kwargs)
    temp=bn_relu(net.conv4_3,inplace=True)
    net.conv4_4 = L.Convolution(temp, num_output=256, pad=1,kernel_size=3, **kwargs)    
    temp6=L.Eltwise(temp5,net.conv4_4)
	
    temp = bn_relu(temp6)
    net.conv4_5 = L.Convolution(temp, num_output=256, pad=1,kernel_size=3, **kwargs)
    temp=bn_relu(net.conv4_5,inplace=True)
    net.conv4_6 = L.Convolution(temp, num_output=256, pad=1,kernel_size=3, **kwargs)    
    temp331=L.Eltwise(temp6,net.conv4_6)
	
    temp = bn_relu(temp331)
    net.conv4_7 = L.Convolution(temp, num_output=256, pad=1,kernel_size=3, **kwargs)
    temp=bn_relu(net.conv4_7,inplace=True)
    net.conv4_8 = L.Convolution(temp, num_output=256, pad=1,kernel_size=3, **kwargs)    
    temp332=L.Eltwise(temp331,net.conv4_8)
	
    temp = bn_relu(temp332)
    net.conv4_9 = L.Convolution(temp, num_output=256, pad=1,kernel_size=3, **kwargs)
    temp=bn_relu(net.conv4_9,inplace=True)
    net.conv4_10 = L.Convolution(temp, num_output=256, pad=1,kernel_size=3, **kwargs)    
    temp333=L.Eltwise(temp332,net.conv4_10)
	
    temp = bn_relu(temp333)
    net.conv4_11 = L.Convolution(temp, num_output=256, pad=1,kernel_size=3, **kwargs)
    temp=bn_relu(net.conv4_11,inplace=True)
    net.conv4_12 = L.Convolution(temp, num_output=256, pad=1,kernel_size=3, **kwargs)    
    temp334=L.Eltwise(temp333,net.conv4_12)

	#block4
    temp = bn_relu(temp334)
    match3=L.Convolution(temp, num_output=512, pad=0,kernel_size=1, stride=2, **kwargs)
    net.conv5_1 = L.Convolution(temp, num_output=512, pad=1,kernel_size=3,stride=2, **kwargs)
    temp=bn_relu(net.conv5_1,inplace=True)
    net.conv5_2 = L.Convolution(temp, num_output=512, pad=1,kernel_size=3, **kwargs)
    temp7=L.Eltwise(match3,net.conv5_2)

    temp = bn_relu(temp7)
    net.conv5_3 = L.Convolution(temp, num_output=512, pad=1,kernel_size=3, **kwargs)
    temp=bn_relu(net.conv5_3,inplace=True)
    net.conv5_4 = L.Convolution(temp, num_output=512, pad=1,kernel_size=3, **kwargs)    
    temp8=L.Eltwise(temp7,net.conv5_4)
	
    temp = bn_relu(temp8)
    net.conv5_5 = L.Convolution(temp, num_output=512, pad=1,kernel_size=3, **kwargs)
    temp=bn_relu(net.conv5_5,inplace=True)
    net.conv5_6 = L.Convolution(temp, num_output=512, pad=1,kernel_size=3, **kwargs)    
    temp9=L.Eltwise(temp8,net.conv5_6)	
	
    temp=bn_relu(temp9)
    match4=L.Convolution(temp, num_output=128, pad=0 , kernel_size=1, stride=2,**kwargs)
    net.conv6_1 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3,stride=2 ,**kwargs)
    temp=bn_relu(net.conv6_1,inplace=True)
    net.conv6_2 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3, **kwargs)
    temp10=L.Eltwise(match4,net.conv6_2)
	
    temp=bn_relu(temp10)
    match5=L.Convolution(temp, num_output=128, pad=0 , kernel_size=1, stride=2,**kwargs)
    net.conv7_1 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3,stride=2 ,**kwargs)
    temp=bn_relu(net.conv7_1,inplace=True)
    net.conv7_2 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3, **kwargs)
    temp11=L.Eltwise(match5,net.conv7_2)	
	
    temp=bn_relu(temp11)
    net.conv8_1 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3,stride=1 ,**kwargs)
    temp=bn_relu(net.conv8_1,inplace=True)
    net.conv8_2 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3, **kwargs)
    temp12=L.Eltwise(temp11,net.conv8_2)

    temp=bn_relu(temp12)
    net.conv9_1 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3,stride=1 ,**kwargs)
    temp=bn_relu(net.conv9_1,inplace=True)
    net.conv9_2 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3, **kwargs)
    temp13=L.Eltwise(temp12,net.conv9_2)    	
	


    return net		
	
def Resnet18rootV2(net, from_layer,freeze_layers=[]):
    def bn_relu(bottom, inplace=True):
	...
	#block1
    temp = bn_relu(net.pool1,inplace=True)
    net.conv2_1 = L.Convolution(temp, num_output=64, pad=1,kernel_size=3, **kwargs)
    temp=bn_relu(net.conv2_1,inplace=True)
    net.conv2_2 = L.Convolution(temp, num_output=64, pad=1,kernel_size=3, **kwargs)
    temp1=L.Eltwise(net.pool1,net.conv2_2)

    temp = bn_relu(temp1,inplace=True)
    net.conv2_3 = L.Convolution(temp, num_output=64, pad=1,kernel_size=3, **kwargs)
    temp=bn_relu(net.conv2_3,inplace=True)
    net.conv2_4 = L.Convolution(temp, num_output=64, pad=1,kernel_size=3, **kwargs)    
    temp2=L.Eltwise(temp1,net.conv2_4)
	...
	#block2
    temp = bn_relu(temp2,inplace=True)
    match1=L.Convolution(temp, num_output=128, pad=0 , kernel_size=1, stride=2,**kwargs)
    net.conv3_1 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3,stride=2 ,**kwargs)
    temp=bn_relu(net.conv3_1,inplace=True)
    net.conv3_2 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3, **kwargs)
    ...
    temp = bn_relu(temp3,inplace=True)
    net.conv3_3 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3, **kwargs)
    temp=bn_relu(net.conv3_3,inplace=True)
    net.conv3_4 = L.Convolution(temp, num_output=128, pad=1,kernel_size=3, **kwargs)    
    temp4=L.Eltwise(temp3,net.conv3_4)
	...
	#block3
    temp = bn_relu(temp4,inplace=True)
    match2=L.Convolution(temp, num_output=256, pad=0,kernel_size=1, stride=2, **kwargs)
    net.conv4_1 = L.Convolution(temp, num_output=256, pad=1,kernel_size=3, stride=2, **kwargs)
    temp=bn_relu(net.conv4_1,inplace=True)
    net.conv4_2 = L.Convolution(temp, num_output=256, pad=1,kernel_size=3, **kwargs)
    ...
    temp = bn_relu(temp5,inplace=True)
    net.conv4_3 = L.Convolution(temp, num_output=256, pad=1,kernel_size=3, **kwargs)
    temp=bn_relu(net.conv4_3,inplace=True)
    net.conv4_4 = L.Convolution(temp, num_output=256, pad=1,kernel_size=3, **kwargs)    
    temp6=L.Eltwise(temp5,net.conv4_4)
	...
	#block4
    temp = bn_relu(temp6,inplace=True)
    match3=L.Convolution(temp, num_output=512, pad=0,kernel_size=1, stride=2, **kwargs)
    net.conv5_1 = L.Convolution(temp, num_output=512, pad=1,kernel_size=3,stride=2, **kwargs)
    temp=bn_relu(net.conv5_1,inplace=True)
    net.conv5_2 = L.Convolution(temp, num_output=512, pad=1,kernel_size=3, **kwargs)
    ...
    temp = bn_relu(temp7,inplace=True)
    net.conv5_3 = L.Convolution(temp, num_output=512, pad=1,kernel_size=3, **kwargs)
    temp=bn_relu(net.conv5_3,inplace=True)
    net.conv5_4 = L.Convolution(temp, num_output=512, pad=1,kernel_size=3, **kwargs)    
    temp8=L.Eltwise(temp7,net.conv5_4)
	
    temp=bn_relu(temp8,inplace=True)
    ...
    def bn_relu_conv(bottom, ks, nout, stride, pad, dropout, inplace=False):
        batch_norm = L.BatchNorm(bottom, in_place=inplace,
                                 param=[dict(lr_mult=0, decay_mult=0), dict(lr_mult=0, decay_mult=0),
                                        dict(lr_mult=0, decay_mult=0)])
        scale = L.Scale(batch_norm, bias_term=True, in_place=True, filler=dict(value=1), bias_filler=dict(value=0))
        relu = L.ReLU(scale, in_place=True)
        conv = L.Convolution(relu, kernel_size=ks, stride=stride,num_output=nout, pad=pad, **kwargs)
        if dropout > 0:
    ...
    def add_bl_layer(bottom, num_filter, dropout, width):
        ...
        conv = bn_relu_conv(conv, ks=3, nout=num_filter, stride=1, pad=1, dropout=dropout,inplace=True)
    ...
    # Stem
    model = L.Convolution(net[from_layer], kernel_size=3, stride=1, num_output=64, pad=1, **kwargs) # output: 150x150
    model = L.BatchNorm(model, in_place=True, param=[dict(lr_mult=0, decay_mult=0), dict(lr_mult=0, decay_mult=0),
                                                      dict(lr_mult=0, decay_mult=0)])
    ...
    model = L.Convolution(model, kernel_size=3, stride=1, num_output=64,pad=1, **kwargs)
    model = L.BatchNorm(model, in_place=True, param=[dict(lr_mult=0, decay_mult=0), dict(lr_mult=0, decay_mult=0),
                                                      dict(lr_mult=0, decay_mult=0)])
    model = L.Scale(model, bias_term=True, in_place=True, filler=dict(value=1), bias_filler=dict(value=0))
    model = L.ReLU(model, in_place=True)
    ...
    for i in range(3):
        ...
        nchannels += growth_rate
    nchannels = int(nchannels / times)
    model = transition_w_o_pooling(model, 192, dropout)  # 38x38
    ...
    for i in range(4):
        ...
        nchannels += growth_rate
    nchannels = int(nchannels / times)
    model1 = transition_w_o_pooling(model1, 384, dropout)  # without pooling
    net.First = model1
    model1 = L.Pooling(model1, pool=P.Pooling.MAX, kernel_size=2, stride=2)	
	
    for i in range(3):
        model1 = add_bl_layer(model1, growth_rate, dropout, 4)
        nchannels += growth_rate
    # nchannels = int(nchannels / times)
    model1 = transition_w_o_pooling(model1, 512, dropout)  # without pooling
    net.Second = model1
    ...
    # name = 'Second'
    #AddExtraLayers
    net.conv6_1 = bn_relu_conv(model1, ks=1, nout=256, stride=1, pad=0, dropout=dropout,inplace=True)
    net.Third = bn_relu_conv(net.conv6_1, ks=3, nout=512, stride=2, pad=1, dropout=dropout,inplace=True)

    net.conv7_1 = bn_relu_conv(net.Third, ks=1, nout=128, stride=1, pad=0, dropout=dropout,inplace=True)
    net.Fourth = bn_relu_conv(net.conv7_1, ks=3, nout=256, stride=2, pad=1, dropout=dropout,inplace=True)

    net.conv8_1 = bn_relu_conv(net.Fourth, ks=1, nout=128, stride=1, pad=0, dropout=dropout,inplace=True)
    net.Fifth = bn_relu_conv(net.conv8_1, ks=3, nout=256, stride=1, pad=0, dropout=dropout,inplace=True)

    net.conv9_1 = bn_relu_conv(net.Fifth, ks=1, nout=128, stride=1, pad=0, dropout=dropout,inplace=True)
    net.Sixth = bn_relu_conv(net.conv9_1, ks=3, nout=256, stride=1, pad=0, dropout=dropout,inplace=True)
...
def ResNet50Body(net, from_layer, use_pool5=True, use_dilation_conv5=False, **bn_param):
    ...
    scale_prefix = 'scale_'
    scale_postfix = ''
    ConvBNLayer(net, from_layer, 'conv1', use_bn=True, use_relu=True,bbbb=True,
        num_output=64, kernel_size=7, pad=3, stride=2,
        conv_prefix=conv_prefix, conv_postfix=conv_postfix,
        bn_prefix=bn_prefix, bn_postfix=bn_postfix,
        scale_prefix=scale_prefix, scale_postfix=scale_postfix, **bn_param)
